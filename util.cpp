...
GLint load_shader(GLenum type, const std::string &file){
	...
	std::string src = load_shader_file(file, file_names);
...
GLuint load_texture(const std::string &file, size_t *width, size_t *height){
	int x, y, n;
...
GLuint load_texture_array(const std::vector<std::string> &files, size_t *w, size_t *h){
	assert(!files.empty());
	int x, y, n;
